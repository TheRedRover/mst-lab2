...
public class WumpusWorldAgent extends Agent {
    ...
    private Dungeon cave;
    private boolean isWumpusAlive = true;
    private boolean isGoldGrabbed;
    private AgentPos agentPosition;
    private boolean hasArrow = true;

    public WumpusWorldAgent(Dungeon cave) {
        this.cave = cave;
    }

    public WumpusWorldAgent() {
        this(new Dungeon(4, 4, Constants.INITIAL_WUMPUS_CAVE));
    }

    public Dungeon getCave() {
        return cave;
    }
    ...
    public AgentPos getAgentPosition() {
        return agentPosition;
    }

    @Override
    protected void setup() {
        System.out.println("Wumpus world [" + getAID().getName() + "] is ready.");
        ...
        public void action() {
            ...
            if (msg != null) {
                ...
                if (message.equals(Constants.GAME_INFORMATION)) {
                    ...
                    AgentFeelings current = getPerceptSeenBy();
    ...
    private boolean isAgentFacingWumpus(AgentPos pos) {
    ...
    public AgentFeelings getPerceptSeenBy() {
        AgentFeelings result = new AgentFeelings();
        AgentPos pos = agentPosition;
        ...
        result.setStench(false);
        result.setBreeze(false);

        adjacentRoomsFull.addAll(adjacentRooms);
        for (Room r : adjacentRoomsFull) {
            if (r.equals(cave.getWumpus()))
                result.setStench(true);
            if (cave.isPit(r))
                result.setBreeze(true);
        }
        if (pos.getRoom().equals(cave.getGold()))
            result.setGlitter(true);
        else
            result.setGlitter(false);
        if (!isWumpusAlive)
            result.setScream(true);
        else 
            result.setScream(false);
        return result;
    }

    private void reset(){
        cave = new Dungeon(4, 4, Constants.INITIAL_WUMPUS_CAVE);
        agentPosition = cave.getStart();
        isWumpusAlive = true;
        isGoldGrabbed = false;
        hasArrow = true;
    ...
    final static class Constants {
        ...
        static final String INITIAL_WUMPUS_CAVE = "S . B P W S . B S . B . . B P B ";
        ...
        static final String ACTION_PROPOSAL = "You should ";
